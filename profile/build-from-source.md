...
Готовые образы `voltara13/*:latest` покрывают 99 % сценариев. Этот документ нужен, если вы правите форк сами или хотите выпустить свой релиз. Цепочка сборки имеет порядок (xray-core ← node ← backend+frontend), потому что каждый юнит ниже забирает образ/артефакт верхнего по пин-файлу.
...
Два файла фиксируют upstream-теги между юнитами — нарушение пина значит «нужно перепересобрать всё ниже по цепочке»:
...
Перед `docker build` проверьте, что чекаут зависимости соответствует пину.
...
Dockerfile — `.github/docker/Dockerfile`. Альтернатива — готовые бинарники в релизах `Fedarisha/xray-builds` (`Xray-linux-64.zip`, `Xray-linux-arm64-v8a.zip`, Windows/macOS/Android).
...
```bash
CGO_ENABLED=0 go build -trimpath -ldflags "-s -w" -o ../bin/xray ./main
```

Требует Go ≥ 1.26 (см. `go.mod`).
...
В node живут модули fedarisha: `src/modules/fedarisha-pak/*` (`provision-user`, `revoke-user`, `probe-user`) и хендлер `handler.service.ts`, который добавляет/удаляет fedarisha-аккаунты в живой xray-runtime.
...
gh release create vX.Y.Z-fed.N remnawave-frontend.zip
```

> **Внимание про zip.** Backend Dockerfile распаковывает архив в `frontend_temp/` и затем `COPY --from=frontend /opt/frontend/frontend_temp/dist ./frontend`. То есть zip обязан содержать верхний каталог `dist/`. Если запаковать `( cd dist && zip -r ../x.zip . )` — backend упадёт на следующем `curl -o frontend_temp/dist/assets/...` с `No such file or directory`.
...
Если frontend-репо приватный — пробросьте токен:

```bash
docker build --secret id=clone_token,src=/tmp/clone_token ...
```

В backend живёт `src/modules/fedarisha-provisioning/*` и контракт node-вызовов `src/common/axios/fedarisha-node-contract.ts` (`/node/fedarisha/{provision,revoke,probe}-user`) — нода должна уметь отвечать на эти ручки.
