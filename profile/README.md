...
## 6. S3-провайдеры для PAK-токенов

Блок `settings.storage` в fedarisha-inbound выбирает, как node выдаёт пользователю prefix-scoped S3-доступ. Поддерживаются два провайдера, переключение по полю `type`. Реализация — `node/src/modules/fedarisha-pak/`.

### `type: "vkcloud-pak"` (по умолчанию)

VK Cloud Prefix Access Keys (PAK). Node создаёт PAK через S3-расширение `?prefixAccess` мастер-ключами.
...
Ограничения VK Cloud: PAK-имена живут **в нэймспейсе мастер-аккаунта, не бакета** — два fedarisha-inbound на одном мастере и разных бакетах коллидируют по `UserAlreadyExists`. Node автоматически добавляет `sha1(inboundTag)[:8]` к имени PAK, так что коллизий нет, но один мастер-ключ → один логический набор пользователей.

### `type: "selectel-iam"`

Selectel S3 + IAM service users. Node создаёт по одному IAM service user на (panel-user, inbound), привязывает им S3-ключи, и поддерживает bucket policy с per-user statement-ами.
...
Под капотом (подтверждено against `vlt-test` 2026-05-24):
...
- лимит **20 KB на policy** ⇒ ~100–150 активных пользователей на бакет; масштабирование — через несколько inbound с разными бакетами;
