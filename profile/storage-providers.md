...
Блок `settings.storage` в fedarisha-инбаунде решает три вещи: **где** живёт бакет, **какими** мастер-ключами нода в него ходит, и **как** она нарезает доступ для каждого юзера.

Третий пункт — основной. Способы нарезки три, выбираются полем `type`. От него зависит, как сильно вы изолируете пользователей друг от друга и сколько S3/IAM-вызовов нужно на каждую активацию.

> Параметры, общие для всех провайдеров (`bucket`, `endpoint`, `region`, `prefix`, `sessionsDir`), описаны в [inbound-config.md](inbound-config.md). Этот документ — только про сам `type` и его особенности.

## Какой выбрать

```
есть один доверенный пользователь, для теста?
    → "static"   (никаких provision-вызовов, все ходят под мастером)
...
self-hosted S3 (MinIO/Garage) без credentials API?
