...
**Транспортные поля (читает xray-core).** Описывают backend и креды для подключения.

| Поле | Тип | Обяз. | Описание |
| --- | --- | --- | --- |
| `type` | string | да | `s3` \| `local`. Регистр не важен. Если пусто — fallback на `s3` (когда задан `bucket`) или `local` (когда задан `localDir`); иначе xray-core падает с `unsupported storage type ""`. |
| `bucket` | string | да* | Имя S3-бакета. Обязателен для `type: s3`. |
...
| `accessKey` / `secretKey` | string | да | Master-ключи. С panel (`authType: vkcloud-pak`/`static`) — реальные рабочие ключи; `selectel-iam` — только для `PutBucketPolicy`. |
| `sessionsDir` | string | нет | Поддиректорий для активных сессий внутри `prefix`. Дефолт — корень `prefix`. |
| `localDir` | string | нет | Альтернатива S3 — локальная директория (`type: local`). Только для отладки на одной машине. |

**Поля для node-side PAK-провайдеров (читает только NestJS-нода).** Парсер xray-core (`infra/conf/fedarisha.go`) их молча игнорирует — без панели можно не задавать.

| Поле | Тип | Обяз. | Описание |
| --- | --- | --- | --- |
| `authType` | string | для panel | `vkcloud-pak` \| `selectel-iam` \| `static` — выбирает PAK-провайдера. Обязателен при работе через Remnawave-стек, требует `type: "s3"`. См. [storage-providers.md](storage-providers.md). |
...
- `storage.type` всегда `"s3"` (клиентский xray умеет только этот транспорт).
- `storage.authType` НЕ передаётся (это node-side понятие — у клиента нет PAK-провайдеров).
